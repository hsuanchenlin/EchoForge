...
if [[ $? -eq 0 ]] && [[ ! "$BUILD_OUTPUT" =~ "BUILD FAILED" ]]; then
    ...
    xattr -d com.apple.quarantine ./Build/Build/Products/Debug/EchoForge.app 2>/dev/null || true
    # Run the app and show logs
    ./Build/Build/Products/Debug/EchoForge.app/Contents/MacOS/EchoForge
else
    echo "Build failed!"
    exit 1
fi 