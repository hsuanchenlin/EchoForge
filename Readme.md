...
- 📖 Personal terms dictionary for deterministic replacements, preferred spellings, names, and protected text. With the Whisper engine your terms are also shown to the recognizer before it decodes, so a name is more often written your way in the first place ([details](docs/personal-terms.md))
- 🧠 App vocabulary - optionally primes the Whisper engine with vocabulary and punctuation matching the app you are dictating into (e.g., camelCase for code editors, formal sign-offs for Mail). Reads only the bundle identifier. Off by default ([details](docs/app-vocabulary.md))
- ✨ Style rewriting - grammar and punctuation are polished as you dictate, and you can turn what you said into formal, concise, bulleted or casual writing instead, or write your own prompt. Runs on device (macOS 26 with Apple Intelligence), on by default with the polishing style, and always keeps the original transcript ([details](docs/style-rewriting.md))
...
- 💊 Floating capsule HUD - a pill at the top of the screen with live input level, duration and progress, shown instead of the card beside the cursor. On by default; switch it off for the card ([details](docs/capsule-hud.md))
- ⚡ Live transcription - each phrase is transcribed while you are still speaking: the words appear on the capsule as you go, and the paste is ready almost as soon as you stop. Nothing is pasted early and nothing changes about the result - the same engines, dictionary, script and rewriting, one paste at the end - and if anything on the live path fails the recording is transcribed whole as before. Local engines only. On by default; switch it off for the whole-file decode ([details](docs/live-dictation.md))
...
- ↩️ Spoken corrections - say "scratch that", "delete the last sentence", "replace Friday with Monday", or "start over" mid-dictation to edit what you just said. It also optionally prunes hesitation sounds like "um" and "uh". Off by default, works only for live dictation, and keeps the original transcript untouched in History ([details](docs/spoken-corrections.md))
...
- 🖥️ `echoforge`, a command-line tool - start the app, transcribe a file (`echoforge transcribe meeting.m4a`), see its version and what it is doing, search your own history, print your settings, read what macOS logged about it, and check for or install an update, all from a terminal or a script. Local and read-only apart from the update and handing the file to the app; every command speaks `--json`. Installed via Homebrew rather than shipped in the DMG ([details](docs/cli.md))
...
- [x] Partial transcription updates for supported engines (Capsule HUD)
