...
- 🖥️ `echoforge`, a command-line tool - start the app, see its version and what it is doing, search your own history, print your settings, read what macOS logged about it, and check for or install an update, all from a terminal or a script. Local and read-only apart from the update, which uses the app's own verified updater; every command speaks `--json`. Built from this repository rather than shipped in the DMG ([details](docs/cli.md))
